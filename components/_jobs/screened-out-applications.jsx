...
export default function ScreenedOutApplications() {
...
const screenedOutApplications = data?.filter((app) => app.status === "screened_out");

const filteredApplications = screenedOutApplications
    ?.filter((app) => {
 ...
 const updateApplicationStatus = async (id,status) => {
  ...
  return (
    <div>
        <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${isUpdating && "blur-2xl opacity-20"}`}>
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Screened Out Applicants</h1>
          <p className="text-muted-foreground">
            Manage {filteredApplications?.length} screened out applications{filteredApplications?.length !== 1 ? "s" : ""} across all
            positions
          </p>
        </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">No screened out applications</h3>
